...
class LlumnixClientVLLM:
    def __init__(self, entrypoints_context: EntrypointsContext):
        ...
        self.request_output_queue: QueueServerBase = (
            entrypoints_context.request_output_queue
        )
    ...
    async def generate(
        ...
        **kwargs
        ...
        try:
            await self._generate_by_manager(
                request_id, server_info_copy, prompt, sampling_params, *args, **kwargs
            )
            self.manager_available = True
            ...
            await self._generate_by_instance(
                request_id, server_info_copy, prompt, sampling_params, *args, **kwargs
            )

        return results_generator

    async def _generate_by_manager(
        self,
        request_id: str,
        ...
        **kwargs
        ...
        await self.manager.generate.remote(
            request_id, server_info, prompt, sampling_params, *args, **kwargs
        )

    async def _generate_by_instance(
        self,
        request_id: str,
        server_info: ServerInfo,
        prompt: str,
        ...
        **kwargs
        ...
        try:
            if self.instance_num_requests:
                ...
                logger.warning(
                    "Manager is unavailable temporarily, dispatch request {} to instance {}".format(
                        request_id, instance_id
                    )
                )
            else:
                logger.warning(
                    "Manager is unavailable temporarily, but there is no instance behind this api server, "
                    "sleep {}s, waiting for manager available".format(
                        WAIT_MANAGER_INTERVAL
                    )
                )
                await asyncio.sleep(WAIT_MANAGER_INTERVAL)
                return await asyncio.create_task(
                    self.generate(prompt, sampling_params, request_id, *args, **kwargs)
                )
            ...
            if instance_id in self.instances:
                ...
                if instance_id in self.instances:
                    ...
                    logger.warning(
                        "instance {} is not in self.instances".format(instance_id)
                    )
                if instance_id in self.instance_num_requests:
                    del self.instance_num_requests[instance_id]
                else:
                    logger.warning(
                        "instance {} is not in self.instance_num_requests".format(
                            instance_id
                        )
                    )
                return await asyncio.create_task(
    ...
    async def get_request_outputs_loop(self):
        while True:
            ...
            set_timestamp(
                request_outputs, "api_server_get_queue_timestamp", time.time()
            )
