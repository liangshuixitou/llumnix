...
class DispatchLoadComputation(LoadComputationStrategy):
    def compute_instance_load(self, instance_info: InstanceInfo) -> float:
        ...
        if self.load_metric == "usage_ratio":
            ...
            max_used_gpu_blocks = (
                instance_info.num_total_gpu_blocks - 15000
                if instance_info.num_total_gpu_blocks > 20000
                else 10000 + 125 * instance_info.num_waiting_requests
            )
            ...
            instance_load = -(num_available_gpu_blocks * throughput) / (
                min(num_requests, max_running_requests)
            )
