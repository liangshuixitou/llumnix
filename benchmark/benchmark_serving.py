...
async def async_request_gen(
    generator, qps: float, distribution="uniform", coefficient_variation: float = 0.0
):
    while True:
        try:
            item = next(generator)
            ...
            if distribution != "burst":
                await asyncio.sleep(
                    get_wait_time(1.0 / qps, distribution, coefficient_variation)
                )
        except StopIteration:
            return
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        try:
            async with session.post(
                f"http://{ip_ports[server_id]}/generate_benchmark", json=request_dict
            ) as resp:
                if verbose:
                    print("Done")
...
def calculate_throughput(
    ...
    all_total_tokens = [
        all_prompt_lens[i] + all_response_lens[i] for i in range(len(all_prompt_lens))
    ]
    all_waiting_latencies = [
        all_e2e_latencies[i] - all_inference_latencies[i]
        for i in range(len(all_e2e_latencies))
    ]
...
def plot_latency_cdf(req_latencies, prefill_latencies, decode_latencies, log_filename):
    ...
    def plot_single(ax, latencies, is_prefill=False):
        ...
        ax.text(
            p80,
            ax.get_ylim()[0] + 0.10 * (ax.get_ylim()[1] - ax.get_ylim()[0]),
            f"{p80:.2f}",
            va="bottom",
            ha="right",
            color="green",
        ...
        ax.text(
            p999,
            ax.get_ylim()[0] + 0.25 * (ax.get_ylim()[1] - ax.get_ylim()[0]),
            f"{p999:.2f}",
            va="bottom",
            ha="right",
            color="gray",
        ...
        mean_percentage = (
            cumsum[np.where(bin_edges[:-1] <= mean)][-1] / np.sum(hist) * 100
        )
        ax.axvline(
            mean_value, color="black", linestyle="-", label="mean={:.2f}".format(mean)
        )
        ax.text(
            mean_value,
            mean_percentage,
...
def save_all_decode_token_latencies_npy(
    all_token_latencies: List[np.ndarray], log_filename
):
    ...
    def measure(self, f):
        async def measured(*args, **kwargs):
            ...
            if "per_token_latency" in output:
                ...
                mean_decode_token_latency = (
                    0 if len(lat_arr) == 1 else np.mean(lat_arr[1:, 1])
                )
...
def sample_burstgpt_request(
    ...
    for idx in sampled_ids:
        if (
            request_tokens[idx] + response_tokens[idx] < max_seqlen
            and request_tokens[idx] > 0
            and response_tokens[idx] > 0
        ):
...
def main():
    ...
    parser.add_argument(
        "--variable_prompt_lens_distribution",
        choices=["uniform", "exponential", "capped_exponential", "zipf"],
        default="uniform",
    )
    ...
    parser.add_argument(
        "--distribution",
        choices=["burst", "uniform", "poisson", "gamma"],
        default="poisson",
    )
    ...
    group.add_argument(
        "--dataset_type", type=str, choices=["sharegpt", "burstgpt", "arxiv"]
    )
